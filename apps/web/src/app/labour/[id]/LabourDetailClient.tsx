...
export default function LabourDetailClient({ profile: p, reviews, serviceItems }: Props) {
  ...
  useEffect(() => {
    checkReviewEligibility('LABOUR', p.id).then(setEligibility).catch(() => setEligibility(null));
  }, [p.id]);
  ...
  async function handleSendMessage() {
    ...
    try {
      ...
      if (err instanceof NotAuthenticatedError) {
        router.push(`/login?redirect=/labour/${p.id}`);
      } else {
  ...
  useEffect(() => {
    trackEvent('view_item', { item_id: p.id, item_category: 'labour', item_name: p.name });
    ...
    <div className="min-h-screen bg-[#FDF8F5]">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center gap-2 text-xs text-[#A08070] mb-6">
          ...
          <span>›</span>
          <Link href="/labour" className="hover:text-[#C0593A]">Labour</Link>
          <span>›</span>
          ...
          <div className="flex-1 min-w-0 space-y-5">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="px-6 pb-6">
                ...
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <span className="text-xs bg-[#FAEEE9] text-[#C0593A] border border-[#E8C4B0] px-3 py-1 rounded-full font-semibold">{p.skillType}</span>
                  {p.availability
            ...
            <Section title="Specialisation">
              <div className="flex flex-wrap gap-2">
                <span className="text-sm bg-[#FAEEE9] text-[#C0593A] border border-[#E8C4B0] px-3 py-1.5 rounded-full font-medium">{p.skillType}</span>
              </div>
            </Section>
            ...
            <Section title="Portfolio">
              <PortfolioGallery profileType="labour" profileId={p.id} />
            </Section>
            ...
            <div className="sticky top-24 bg-white rounded-2xl shadow-lg border border-gray-100 p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#A08070]">{p.skillType}</p>
                </div>
              </div>

              <div>
                {p.dailyRate && p.dailyRate > 0 ? (
                  <>
                    <p className="text-2xl font-bold text-[#C0593A]">₹{p.dailyRate.toLocaleString('en-IN')}</p>
                    <p className="text-xs text-[#A08070]">per day (negotiable)</p>
                  </>
                ) : (
              ...
              {p.availability ? (
                <div className="flex items-center gap-2 text-sm text-green-600 bg-green-50 border border-green-100 rounded-lg px-3 py-2">
                  <span>🟢</span><span className="font-medium">Available for work</span>
                </div>
              ) : (
        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-[#EBE0D8] px-4 py-3 flex gap-3 z-30 shadow-lg">
          <div className="flex-1">
            <p className="text-xs text-[#A08070]">Daily rate</p>
            <p className="text-base font-bold text-[#C0593A]">
              {p.dailyRate && p.dailyRate > 0 ? `₹${p.dailyRate.toLocaleString('en-IN')}/day` : 'Rate on request'}
            </p>
          </div>
          <button onClick={() => openBookingFor()}
