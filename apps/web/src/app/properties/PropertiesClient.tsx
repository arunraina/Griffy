...
export default function PropertiesClient({ initialProperties }: { initialProperties: Property[] }) {
  return <Suspense><PropertiesInner initialProperties={initialProperties} /></Suspense>;
}

function PropertiesInner({ initialProperties }: { initialProperties: Property[] }) {
  ...
  const filtered = useMemo(() => {
    if (tab === 'land') return [];
    let list = initialProperties.filter(p => p.tab === (tab === 'new' ? 'new' : tab));
    ...
    return list;
  }, [initialProperties, tab, search, sort, selTypes, selBhk, selFurn, selCity, maxPrice]);
