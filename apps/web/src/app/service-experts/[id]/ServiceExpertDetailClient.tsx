...
interface Props {
  profile: ServiceExpertProfile;
  reviews: Review[];
  serviceItems: PublicServiceItem[];
}

export default function ServiceExpertDetailClient({ profile: p, reviews, serviceItems }: Props) {
  ...
  useEffect(() => {
    trackEvent('view_item', { item_id: p.id, item_category: 'service_expert', item_name: p.name });
    ...
    <div className="min-h-screen bg-[#FDF8F5]">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center gap-2 text-xs text-[#A08070] mb-6">
          ...
          <span>›</span>
          <Link href="/service-experts" className="hover:text-[#C0593A]">Service Experts</Link>
          <span>›</span>
        ...
        <div className="flex gap-8 items-start">
          ...
          <div className="flex-1 min-w-0 space-y-5">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="px-6 pb-6">
                ...
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <span className="text-xs bg-blue-100 text-blue-700 border border-blue-200 px-3 py-1 rounded-full font-semibold">{p.expertiseType}</span>
                  {p.availability
            ...
            <Section title="Expertise &amp; Qualifications">
              <div className="mb-3">
                <p className="text-xs font-bold text-[#6B5248] uppercase tracking-widest mb-2">Expertise Type</p>
                <span className="text-sm bg-blue-100 text-blue-700 border border-blue-200 px-3 py-1.5 rounded-full font-medium">{p.expertiseType}</span>
              </div>
              {p.qualifications.length > 0 && (
                <div>
                  <p className="text-xs font-bold text-[#6B5248] uppercase tracking-widest mb-2">Qualifications</p>
                  <div className="flex flex-wrap gap-2">
                    {p.qualifications.map(q => (
                      <span key={q} className="text-sm bg-[#FAEEE9] text-[#C0593A] border border-[#E8C4B0] px-3 py-1.5 rounded-full font-medium">{q}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            {p.serviceCities.length > 0 && (
              <Section title="Service Areas">
                <div className="flex flex-wrap gap-2">
                  {p.serviceCities.map(city => (
                    <span key={city} className="text-sm bg-white border border-[#EBE0D8] text-[#3D2B22] px-3 py-1.5 rounded-full font-medium">📍 {city}</span>
                  ))}
                </div>
              </Section>
            )}
          ...
          <div className="hidden lg:block w-80 flex-shrink-0">
            <div className="sticky top-24 bg-white rounded-2xl shadow-lg border border-gray-100 p-6 space-y-4">
              ...
              <div>
                {p.consultationFee && p.consultationFee > 0 ? (
                  <>
                    <p className="text-2xl font-bold text-[#C0593A]">₹{p.consultationFee.toLocaleString('en-IN')}</p>
                    <p className="text-xs text-[#A08070]">per consultation session</p>
                  </>
                ) : (
                  <p className="text-base font-semibold text-[#6B5248]">Fee on request</p>
                )}
              </div>

              {p.availability ? (
                <div className="flex items-center gap-2 text-sm text-green-600 bg-green-50 border border-green-100 rounded-lg px-3 py-2">
                  <span>🟢</span><span className="font-medium">Available for consultation</span>
                </div>
              ) : (
              ...
              <div className="border-t border-[#F0E8E2] pt-4 space-y-2.5">
                {[
                  ...
                  ...(p.qualifications.length > 0 ? [{ icon: '🎓', text: p.qualifications[0] }] : []),
                ].map(item => (
        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-[#EBE0D8] px-4 py-3 flex gap-3 z-30 shadow-lg">
          <div className="flex-1">
            <p className="text-xs text-[#A08070]">Consultation fee</p>
            <p className="text-base font-bold text-[#C0593A]">
              {p.consultationFee && p.consultationFee > 0 ? `₹${p.consultationFee.toLocaleString('en-IN')}/session` : 'Fee on request'}
            </p>
          </div>
          <button onClick={() => openBookingFor()}
