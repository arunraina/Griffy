        ...
        <div className="w-full max-w-md">
          ...
          {mode === 'wp-phone' && (
            <>
              ...
              <p className="text-sm text-[#6B5248] text-center mb-8">Enter your WhatsApp number to receive an OTP</p>
              <form onSubmit={sendWhatsappOtp} className="space-y-4">
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? <><SpinSvg />Sending…</> : 'Send OTP on WhatsApp'}
                </button>
              </form>
            </>
          )}
          ...
          {mode === 'wp-otp' && (
            <>
              ...
              <p className="text-sm font-semibold text-[#2C1810] text-center mb-8">+91 {phone} via WhatsApp</p>
              <OtpBoxes digits={digits} inputRefs={inputRefs} loading={loading}
                onDigit={handleDigit} onKeyDown={handleKeyDown} onPaste={handlePaste} />
              {error && <ErrBox>{error}</ErrBox>}
              {loading && <Spinner />}
