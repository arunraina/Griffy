...
const LAND_TYPE_FLAG: Partial<Record<ListingType, string>> = {
  'Residential Plot':  'residential_plot',
  'Agricultural Land': 'agricultural_land',
  'Commercial Plot':   'commercial_plot',
  'Industrial':        'industrial_plot',
};
const TYPES: ListingType[] = (
  ['Residential Plot', 'Agricultural Land', 'Commercial Plot', 'Farm House', 'Industrial'] as ListingType[]
).filter(t => {
  const flagKey = LAND_TYPE_FLAG[t];
  return !flagKey || isSubEnabled('land', flagKey);
});
...
export default function LandClient({ initialListings }: { initialListings: LandListing[] }) {
  return <Suspense><LandInner initialListings={initialListings} /></Suspense>;
}

function LandInner({ initialListings }: { initialListings: LandListing[] }) {
  ...
  const listings = useMemo(() => {
    ...
    return list;
  }, [initialListings, search, sort, selTypes, selStates, selFacings, approvedOnly, ownerOnly, priceMin, priceMax]);
