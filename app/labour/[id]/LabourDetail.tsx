...
import { Star, MapPin, CheckCircle2, Phone, Calendar, Shield, Clock, Award, ChevronRight, MessageSquare, Wrench, Share2 } from "lucide-react";
...
export default function LabourDetailPage() {
  ...
  const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`${name} — ${tradeLabel} on Griffy\n${pageUrl}`)}`;
  ...
  const jsonLd = {
    ...
    image: worker.avatarUrl ?? undefined,
    address: worker.city ? {
      "@type": "PostalAddress",
      addressLocality: worker.city,
      addressRegion: worker.state ?? undefined,
      addressCountry: "IN",
    } : undefined,
  };
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          {showEnquiry && (
            <EnquiryModal
              ...
              targetId={worker.id}
              recipientName={worker.user?.fullName ?? "Worker"}
          ...
          <aside className="space-y-5">
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 sticky top-24">
              ...
              <Link href={`/checkout?type=labour&id=${worker.id}`} className="w-full flex items-center justify-center gap-2 border-2 border-stone-200 hover:border-green-300 text-stone-700 hover:text-green-700 font-semibold py-3 rounded-xl transition-all mb-2">
                Book Now
              </Link>
              <button className="w-full flex items-center justify-center gap-2 border-2 border-stone-200 hover:border-blue-300 text-stone-700 hover:text-blue-600 font-semibold py-3 rounded-xl transition-all mb-2">
                <Phone className="w-4 h-4" /> Call Now
              </button>
              <a href={whatsappUrl} target="_blank" rel="noopener noreferrer"
                className="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20bc5a] text-white font-semibold py-3 rounded-xl transition-all">
