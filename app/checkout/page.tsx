...
export default function CheckoutPage() {
  ...
  async function handlePlaceOrder() {
    ...
    const deliveryAddress = [address.line1, address.line2, address.city, address.state, address.pincode]
      .filter(Boolean).join(", ");
    const token = typeof window !== "undefined" ? localStorage.getItem("griffy_token") ?? "" : "";
    ...
    try {
      ...
      const order = await createRazorpayOrder(grandTotal, token, deliveryAddress);
    ...
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-stone-100 p-8 text-center">
          ...
          <p className="text-sm font-bold text-orange-500 mb-1">Order ID: {orderResult.id}</p>
          {orderResult.paymentId && (
            <p className="text-xs text-stone-400 mb-6">Payment ID: {orderResult.paymentId}</p>
          )}
          {orderResult.paymentMethod === "cod" && (
            <p className="text-xs text-stone-400 mb-6">Cash on Delivery — pay when materials arrive</p>
          )}
          ...
          <div className="bg-stone-50 rounded-2xl p-4 mb-6 text-left space-y-2 text-sm text-stone-600">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>SMS updates on {address.phone}</span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span>
                {orderResult.paymentMethod === "cod"
                  ...
                  : "Payment secured via Griffy Escrow"}
              </span>
            </div>
          </div>

          <div className="flex flex-col gap-3">
            <Link href={`/orders/${orderResult.id}`} className="btn-primary justify-center">
              Track Your Order
            </Link>
      ...
      <div className="bg-white border-b border-stone-100">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-sm text-stone-500">
            ...
            <span
              className={step === "address" ? "text-stone-900 font-medium" : "hover:text-orange-500 cursor-pointer"}
              onClick={() => step !== "address" && setStep("address")}
            >
              Delivery Address
            </span>
            <ChevronRight className="w-4 h-4" />
            <span className={step === "payment" ? "text-stone-900 font-medium" : "text-stone-400"}>
              Payment
            </span>
          </nav>
        </div>
              ...
              return (
                <div key={s} className="flex items-center gap-3">
                  <div className={`flex items-center gap-2 text-sm font-semibold ${active ? "text-orange-500" : done ? "text-green-500" : "text-stone-300"}`}>
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 ${active ? "border-orange-500 bg-orange-50 text-orange-600" : done ? "border-green-500 bg-green-50 text-green-600" : "border-stone-200 text-stone-300"}`}>
                      {done ? "✓" : i + 1}
                    </div>
                    <span className="hidden sm:inline capitalize">{s === "address" ? "Delivery Address" : "Payment"}</span>
                  </div>
                  {i < 1 && <div className={`h-0.5 w-8 ${done ? "bg-green-400" : "bg-stone-200"}`} />}
                </div>
              );
            })}
          </div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border border-stone-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-stone-700 mb-1.5">Mobile Number *</label>
                    <input
                      required type="tel" pattern="[6-9]\d{9}" value={address.phone}
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full border border-stone-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 bg-white"
                    >
                  ...
                  <div>
                    ...
                    <input
                      required pattern="\d{6}" value={address.pincode}
            ...
            {step === "payment" && (
              <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 space-y-5">
                ...
                <div className="space-y-3">
                  {[
                    { id: "upi",  label: "UPI / GPay / PhonePe / Paytm", desc: "Pay instantly via any UPI app. Powered by Razorpay.", icon: "📱" },
                    { id: "card", label: "Credit / Debit Card",           desc: "Visa, Mastercard, RuPay. Secured by Razorpay.", icon: "💳" },
                    { id: "cod",  label: "Cash on Delivery",              desc: "Pay when materials arrive at your site. ₹50 fee applies.", icon: "💵" },
                  ].map((m) => (
                    <label
                      key={m.id}
                ...
                {(paymentMethod === "upi" || paymentMethod === "card") && (
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 flex items-start gap-2 text-sm text-blue-700">
                    <Shield className="w-4 h-4 shrink-0 mt-0.5" />
                    <span>
                      Clicking "Pay Now" opens the <strong>Razorpay secure checkout</strong> — your card/UPI details
                      are handled directly by Razorpay and never stored by Griffy.
                    </span>
                  </div>
                )}
                ...
                {paymentMethod === "cod" && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm text-amber-700">
                    ₹50 COD convenience fee is added. Our delivery partner collects payment on arrival.
                  </div>
                )}

                <div className="bg-green-50 border border-green-100 rounded-xl p-3 flex items-center gap-2 text-sm text-green-700">
                  <Shield className="w-4 h-4 shrink-0" />
                  All payments are escrow-protected. Funds released to supplier only after delivery confirmation.
                </div>
                ...
                >
                  {placing ? (
                    ...
                    <>
                      ...
                      {" · "}₹{(grandTotal + (paymentMethod === "cod" ? 50 : 0)).toLocaleString("en-IN")}
            ...
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-5 sticky top-24">
              ...
              <div className="border-t border-stone-100 pt-4 space-y-2 text-sm">
                ...
                <div className="border-t border-stone-100 pt-2 flex justify-between font-extrabold text-stone-900">
                  <span>Total</span>
                  <span>₹{(grandTotal + (paymentMethod === "cod" ? 50 : 0)).toLocaleString("en-IN")}</span>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-stone-100 flex items-center justify-center gap-3 text-xs text-stone-400">
